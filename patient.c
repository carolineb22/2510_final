...
void addElement(struct Patient newPatient, struct Patient patientList[])
{
    static int count = 0; // Keep track of the number of patients added
    if (count < 50) { // Check if there's space in the array
        patientList[count] = newPatient; // Add the new patient to the array
        count++;
    } else {
...
void printArray(int count, struct Patient patientList[])
{
    ...
    for (int i = 0; i < count; i++)
    {
        printf("\n%d. Patient ID: %d"
               ...
               "\nAge: %d"
               "\nDiagnosis: "
               "\nRoom Number: ", i + 1, patientList[i].patientID,
               patientList[i].name, patientList[i].age,
               patientList[i].diagnosis, patientList[i].roomNumber);
    }
}
